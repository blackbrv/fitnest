...
function ExerciseRow({
  ...
  return (
    <div className="bg-[#1c2433] rounded-xl p-3 flex items-center gap-3 group transition-colors hover:bg-[#202b3d]">
      {/* Number */}
      <div className="shrink-0 h-7 w-7 rounded-lg bg-[#0f1115] flex items-center justify-center text-xs font-bold text-[#a3ff3f]">
        {index + 1}
      </div>

      {/* Icon */}
      <div className="shrink-0 text-[#8b95a5]">
        <Dumbbell size={14} strokeWidth={1.75} />
      </div>

      {/* Name + meta */}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-[#f5f7fa] truncate">{exercise.exerciseName}</p>
        <div className="flex items-center gap-3 mt-0.5 flex-wrap">
          ...
          {!hasReps && !hasDuration && (
            <span className="text-xs text-[#8b95a5]">{exercise.sets} sets</span>
          )}
          {/* Rest */}
      ...
      {reorderable && (
        <div className="shrink-0 flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className={cn(
              'h-5 w-5 rounded flex items-center justify-center text-[#8b95a5] transition-colors',
              'hover:text-[#f5f7fa] hover:bg-white/8 disabled:opacity-30 disabled:cursor-not-allowed',
            )}
          >
            <ChevronUp size={12} />
          </button>
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-white/10 p-6 text-center">
        <p className="text-sm text-[#8b95a5]">No exercises added yet.</p>
      </div>
    )
  }
